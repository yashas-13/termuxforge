...
import { AgentConfig, ToolResult } from "./types.js";
...
export class Agent {
  ...
  private messages: { role: string; content: string; tool_calls?: any }[] = [];
...
`,
    ...
    while (loop++ < maxLoops) {
      ...
      for (const call of message.tool_calls) {
        ...
        if (name === "bash") {
          await this.sessionManager.logCommand(
            session.id,
  ...
  private async scoreSession(sessionId: string) {
